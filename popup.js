    ...
    ***************************************************************************** */
		...
		var slice16$1 = function (view, start, end) {
			...
			return new Uint16Array(
				Array.prototype.slice.call(view, start, end)
			);
		};
		...
		var createTrieFromBase64$1 = function (base64, _byteLength) {
			...
			var data =
				view32[5] === 2
					...
					: slice32$1(
							view32,
							Math.ceil((headerLength + view32[4]) / 4)
					  );
			return new Trie$1(
				view32[0],
				view32[1],
			...
			Trie.prototype.get = function (codePoint) {
				...
				if (codePoint >= 0) {
					...
					if (codePoint < this.highStart) {
						...
						ix = this.index[ix];
						ix +=
							(codePoint >> UTRIE2_SHIFT_2$1) &
							UTRIE2_INDEX_2_MASK$1;
						ix = this.index[ix];
						ix =
							(ix << UTRIE2_INDEX_SHIFT$1) +
							(codePoint & UTRIE2_DATA_MASK$1);
		...
		var codePointsToCharacterClasses = function (codePoints, lineBreak) {
			...
			codePoints.forEach(function (codePoint, index) {
				...
				if (["normal", "auto", "loose"].indexOf(lineBreak) !== -1) {
					...
					if (
						[0x2010, 0x2013, 0x301c, 0x30a0].indexOf(codePoint) !==
						-1
					) {
						indices.push(index);
		...
		var isAdjacentWithSpaceIgnored = function (
			a,
			b,
			currentIndex,
			classTypes
			...
			if (current === SP) {
				...
				while (i > 0) {
					...
					if (
						Array.isArray(a) ? a.indexOf(prev) !== -1 : a === prev
					) {
						var n = currentIndex;
		...
		var processColorStops = function (stops, lineLength) {
			...
			return stops.map(function (_a, i) {
				...
				return {
					color: color,
					stop: Math.max(
						Math.min(1, processStops[i] / lineLength),
						0
					),
				};
			});
			...
			parse: function (_context, tokens) {
				...
				for (var i = 0; i < filtered.length; i++) {
					...
					if (isIdentToken(counter) && counter.value !== "none") {
						var reset =
							next && isNumberToken(next) ? next.number : 0;
		...
		var CSSParsedDeclaration = /** @class */ (function () {
			function CSSParsedDeclaration(context, declaration) {
				...
				this.textDecorationColor = parse(
					context,
					textDecorationColor,
					(_a = declaration.textDecorationColor) !== null &&
						_a !== void 0
						? _a
						: declaration.color
				);
				this.textDecorationLine = parse(
					context,
					textDecorationLine,
					(_b = declaration.textDecorationLine) !== null &&
						_b !== void 0
						? _b
		...
		var GraphemeBreaker = function (str) {
			...
			return {
				next: function () {
					...
					if (
						graphemeBreak !== BREAK_NOT_ALLOWED ||
						index === length
					) {
			...
			function IFrameElementContainer(context, iframe) {
				...
				try {
					if (
						...
						var documentBackgroundColor = iframe.contentWindow
							...
							: COLORS.TRANSPARENT;
						var bodyBackgroundColor = iframe.contentWindow.document
							.body
							? parseColor(
									context,
									getComputedStyle(
		...
		var ARMENIAN = {
			integers: [
				...
				700, 600, 500, 400, 300, 200, 100, 90, 80, 70, 60, 50, 40, 30,
		...
		var HEBREW = {
			integers: [
				...
				400, 300, 200, 100, 90, 80, 70, 60, 50, 40, 30, 20, 19, 18, 17,
					...
					if (token.type === 0 /* STRING_TOKEN */) {
						...
						if (token.name === "attr") {
							...
							if (counter && isIdentToken(counter)) {
								var counterState =
									_this.counters.getCounterValue(
										counter.value
									);
								var counterType =
				...
				this.topLeftContentBox =
					tlh > 0 || tlv > 0
						? getCurvePoints(
								...
								Math.max(
									0,
									tlh - (borderLeftWidth + paddingLeft)
								),
								Math.max(
									0,
									tlv - (borderTopWidth + paddingTop)
								),
								CORNER.TOP_LEFT
						  )
						: new Vector(
				...
				this.bottomLeftContentBox =
					blh > 0 || blv > 0
						? getCurvePoints(
								...
								Math.max(
									0,
									blh - (borderLeftWidth + paddingLeft)
								),
		...
		var ElementPaint = /** @class */ (function () {
			...
			ElementPaint.prototype.getEffects = function (target) {
				...
				while (parent) {
					var croplessEffects = parent.effects.filter(function (
						effect
					) {
					...
					if (
						...
						if (
							parent.container.styles.overflowX !==
							0 /* VISIBLE */
						) {
							var borderBox = calculateBorderBoxPath(
								parent.curves
							);
							var paddingBox = calculatePaddingBoxPath(
								parent.curves
							);
							if (!equalPath(borderBox, paddingBox)) {
								effects.unshift(
			...
			parent.container.elements.forEach(function (child) {
				...
				if (treatAsRealStackingContext || createsStackingContext) {
					...
					if (
						...
						if (order_1 < 0) {
							var index_1 = 0;
							parentStack.negativeZIndex.some(function (
								current,
								i
							) {
								if (
									order_1 >
									current.element.container.styles.zIndex
										.order
								) {
									index_1 = i;
									return false;
							...
							var index_2 = 0;
							parentStack.positiveZIndex.some(function (
								current,
								i
							) {
								if (
		...
		var calculateBackgroundPaintingArea = function (
			backgroundClip,
			element
		) {
			...
			switch (repeat) {
				...
				case 3 /* REPEAT_Y */:
					return [
						...
						new Vector(
							Math.round(
								backgroundPositioningArea.left + x + width
							),
							Math.round(backgroundPositioningArea.top)
						),
						new Vector(
			...
			CanvasRenderer.prototype.renderTextNode = function (text, styles) {
				return __awaiter(this, void 0, void 0, function () {
					...
					return __generator(this, function (_c) {
						...
						(_b = this.fontMetrics.getMetrics(
							fontFamily,
							fontSize
						)),
					...
					return __generator(this, function (_c) {
						switch (_c.label) {
							...
							case 19:
								if (
									paint.listValue &&
									container.styles.listStyleType !==
										-1 /* NONE */
								) {
									fontFamily =
				...
				return __awaiter(this, void 0, void 0, function () {
					...
					return __generator(this, function (_b) {
						switch (_b.label) {
							case 0:
								index =
									container.styles.backgroundImage.length - 1;
					...
					switch (_u.label) {
						case 0:
							...
							windowOptions = {
								...
								scrollX:
									(_h = opts.scrollX) !== null &&
									_h !== void 0
										? _h
										: defaultView.pageXOffset,
								scrollY:
									(_j = opts.scrollY) !== null &&
									_j !== void 0
										? _j
							...
							clonedElement =
								documentCloner.clonedReferenceElement;
							if (!clonedElement) {
								return [
									2 /*return*/,
									Promise.reject(
	...
	html2canvas(htmlContent, {
		...
		date = date.substring(5, date.length - 13);
		...
		for (let i = 0; i < date.length; i++) {
			...
			if (date.charAt(i) == " ") {
				...
				formattedDate += date.charAt(i);
			}
		}
		link.download = "Infinite-Screenshot-" + formattedDate + ".png";
		link.href = dataURL;
		link.click();
	});
	chrome.runtime.sendMessage({ action: "captureDOM", htmlContent });
}
